      ...
      if (interaction.isChatInputCommand()) {
        if (interaction.guild) {
          ...
          const embed = new EmbedBuilder2()
            ...
            .setAuthor({ name: name4, iconURL: target_icon })
            .setFooter({ text: `Server ID: ${guild.id}`, iconURL: target_icon })
            .setTimestamp()
            .addFields({ name: 'Name', value: `${name4}`, inline: false })
...
var enabled_commands = [command_ping, command_server_icon, command_server_info, command_user_avatar, command_user_check_troll, command_user_info, command_verify];
